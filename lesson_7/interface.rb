...
class Interface
	...
	def menu
		menu = [
			...
			{action_title: "Вывести список поездов на станции", index: 4, action: :train_list_on_the_stations},
			...
			{action_title: "Изменить количество вагонов поезда", index: 7, action: :train_carriage_manage},
			{action_title: "Заполнить вагон поезда", index: 8, action: :fill_the_carriage},
			{action_title: "Задать скорость поезду", index: 9, action: :train_change_speed},
			{action_title: "Остановить поезд", index: 10, action: :train_stop},
			{action_title: "Добавить маршрут к поезду", index: 11, action: :assign_route_to_train},
			{action_title: "Перемещать поезд между станциями", index: 12, action: :drive_the_train_betwin_stations},
			{action_title: "Показать информацию о поездах", index: 13, action: :trains_info},
			{action_title: "Показать список вагонов у поезда", index: 14, action: :show_trains_wagons},
			{break_block: "\n"},			
			{action_block: "* УПРАВЛЕНИЕ МАРШРУТАМИ *"},			
			{action_title: "Создать новый маршрут", index: 15, action: :create_new_route},
			{action_title: "Редактировать маршрут", index: 16, action: :change_intermediate_stations_fron_route},
			{action_title: "Показать список станций на маршруте", index: 17, action: :show_route_stations},
			{break_block: "\n"},	
	...
	def add_train_to_station
		loop do
			...
			print stations.empty? ? "Доступных станций нет. Введите '999', чтобы выйти: " : "Введите индекс станции или '999', чтобы выйти : "
			arrival_station_index = gets.chomp.to_i
			system 'clear'
			break if arrival_station_index == 999
			show_accessible_trains
			print trains.empty? ? "Доступных поездов нет. Введите '999', чтобы выйти: " : "Введите индекс поезда, который необходимо принять: "
			...
			break if stations[arrival_station_index].trains.any?
		end
	end
	...
	def departure_train_from_station
		loop do
			system 'clear'
			show_accessible_stations
			print stations.empty? ? "Доступных станций нет. Введите '999', чтобы выйти: " : "Введите индекс станции, где поезда ждут отправления: "
			...
			print "Введите индекс выбранного поезда: "
	...
	def train_list_on_the_stations
		...
		system 'clear'
		block = lambda { |train| puts "Поезд № #{train.number}, тип: #{train.type}, количество вагонов: #{train.wagons.size}."}
	...
	def create_train
		...
		when 'cargo' then create_cargo_train
			...
			system 'clear'
			puts "Задайте корректное значение.\n\n"
	end
	...
	def hook_wagon
		if trains[@carriage_train].type == :cargo && trains[@carriage_train].speed == 0
			...
			volume_insert = gets.chomp.to_i
			cargo_wagon = CargoWagon.new(volume_insert)
			...
			print 'Укажите вместимость вагона: '
			volume_insert = gets.chomp.to_i		
			passenger_wagon = PassengerWagon.new(volume_insert)
